...
@st.cache_data(ttl=300)
def load():
    agg_t = q("SELECT * FROM agg_transaction")
    agg_u = q("SELECT * FROM agg_user")
    agg_i = q("SELECT * FROM agg_insurance")
    map_t = q("SELECT * FROM map_transaction")
    map_u = q("SELECT * FROM map_user")
    map_i = q("SELECT * FROM map_insurance")
    top_t = q("SELECT * FROM top_transaction")
    top_u = q("SELECT * FROM top_user")
    top_i = q("SELECT * FROM top_insurance")
...
with st.sidebar:
    ...
    page = st.radio("🧭 Navigation", ["📊 Overview","💳 Transactions","👥 Users","🛡️ Insurance","🗺️ Geo Maps","💡 Business Cases"], label_visibility="collapsed")
    ...
    sel_qtr   = st.selectbox("🗓️ Quarter", all_qtrs)
    ...
    st.markdown(f"""<div style='margin-top:30px;padding:10px;background:{C};border-radius:10px;border:1px solid {P};font-size:11px;color:#cbb8e8;text-align:center'>
        🟢 Live MySQL · Auto-refresh 5min<br>
        Data: 2018–2024 · 9 Tables
    </div>""", unsafe_allow_html=True)
...
if page == "📊 Overview":
    ...
    ft = filt(agg_t); fu = filt(agg_u); fi = filt(agg_i)
    fm = filt(map_u)
    ...
    ins_cnt     = fi["insurance_count"].sum()
    ins_amt     = fi["insurance_amount"].sum()
    ...
    kpi(r1[2],"📈 Avg Txn Value",      f"₹{avg_txn:,.0f}")
    kpi(r1[3],"👥 Registered Users",   fmtn(total_users))

    r2 = st.columns(4)
    kpi(r2[0],"📱 App Opens",          fmtn(app_opens))
    kpi(r2[1],"🏛️ Active States",      str(states_cnt))
    kpi(r2[2],"🗺️ Districts",          str(dist_cnt))
    kpi(r2[3],"📱 Mobile Brands",      str(brands))

    r3 = st.columns(4)
    kpi(r3[0],"🛡️ Total Policies",     fmtn(ins_cnt))
    kpi(r3[1],"💵 Insurance Premium",  fmt(ins_amt))
    kpi(r3[2],"📋 Avg Premium/Policy", f"₹{avg_prem:,.0f}")
    ...
    f1.update_traces(fill='tozeroy', fillcolor=f"rgba(95,37,159,0.3)")
...
# ── PAGE 2: TRANSACTIONS ─────────────────────────────────────────────
    ...
    t1,t2,t3,t4 = st.tabs(["📈 Trends","🍩 Type Analysis","🏆 State Rankings","🌡️ Heatmap"])
    ...
    with t2:
        ...
        f3 = px.area(ttype_q,x="period",y="transaction_amount",color="transaction_type",title="Txn Type Over Time (Stacked)",
    ...
    with t3:
        ...
        c1,c2 = st.columns(2)
        t10a = st_t.nlargest(10,"transaction_amount")
        t10c = st_t.nlargest(10,"transaction_count")
        f1 = px.bar(t10a,x="transaction_amount",y="state",orientation="h",title="Top 10 States by Amount",
                    color="transaction_amount",color_continuous_scale=CSCALE)
        f2 = px.bar(t10c,x="transaction_count",y="state",orientation="h",title="Top 10 States by Count",
                    color="transaction_count",color_continuous_scale=CSCALE)
        c1.plotly_chart(theme(f1),use_container_width=True)
        c2.plotly_chart(theme(f2),use_container_width=True)
    ...
    with t4:
        ...
        f = px.imshow(heat/1e12,title="State × Year Heatmap (₹T)",color_continuous_scale=CSCALE,
                      labels=dict(color="₹T"),aspect="auto",text_auto=".1f")
...
# ── PAGE 3: USERS ────────────────────────────────────────────────────
    ...
    t1,t2,t3 = st.tabs(["📱 Brand Analysis","🏛️ State Analysis","📈 Growth Trends"])
    ...
    with t3:
        ...
        c1,c2 = st.columns(2)
        f1 = px.area(qu,x="period",y="registered_users",title="User Registration Growth",
                     color_discrete_sequence=[A])
        f2 = px.area(qa,x="period",y="app_opens",title="App Opens Growth",
        ...
        f3 = px.imshow(heat_u/1e6,title="State × Year User Heatmap (Millions)",
...
# ── PAGE 4: INSURANCE ────────────────────────────────────────────────
    ...
    st.markdown(f"<h1 style='color:{G}'>🛡️ Insurance Analytics</h1>", unsafe_allow_html=True)
    fi = filt(agg_i); fmi = filt(map_i)

    t1,t2,t3 = st.tabs(["📈 Growth","🏛️ State View","🗺️ District View"])

    with t1:
        qi = period(fi.groupby(["year","quarter"])[["insurance_count","insurance_amount"]].sum().reset_index())
        qi["avg_prem"] = qi["insurance_amount"]/qi["insurance_count"]
        c1,c2 = st.columns(2)
        f1 = px.bar(qi,x="period",y="insurance_count",title="Quarterly Policy Count",
                    color="insurance_count",color_continuous_scale=CSCALE)
        f2 = px.line(qi,x="period",y="insurance_amount",title="Quarterly Premium Growth",
        ...
        yi = fi.groupby("year")[["insurance_count","insurance_amount"]].sum().reset_index()
        yi["yoy"] = yi["insurance_amount"].pct_change()*100
    ...
    with t2:
        si = fi.groupby("state")[["insurance_count","insurance_amount"]].sum().reset_index()
        c1,c2 = st.columns(2)
        f1 = px.bar(si.nlargest(15,"insurance_count"),x="state",y="insurance_count",
                    title="Top 15 States — Policies",color="insurance_count",color_continuous_scale=CSCALE)
        f2 = px.bar(si.nlargest(15,"insurance_amount"),x="state",y="insurance_amount",
                    title="Top 15 States — Premium",color="insurance_amount",color_continuous_scale=CSCALE)
        f1.update_xaxes(tickangle=45); f2.update_xaxes(tickangle=45)
        c1.plotly_chart(theme(f1),use_container_width=True)
        c2.plotly_chart(theme(f2),use_container_width=True)

        f3 = px.scatter(si,x="insurance_count",y="insurance_amount",text="state",
                        title="Policy Count vs Premium — State Scatter",size="insurance_amount",
                        color="insurance_amount",color_continuous_scale=CSCALE)
        f3.update_traces(textposition="top center",textfont_size=9)
        st.plotly_chart(theme(f3,400),use_container_width=True)

    with t3:
        di = fmi.groupby("district")[["insurance_count","insurance_amount"]].sum().reset_index()
        f1 = px.bar(di.nlargest(15,"insurance_amount"),x="insurance_amount",y="district",
...
# ── PAGE 5: GEO MAPS ─────────────────────────────────────────────────
    ...
    st.markdown(f"<h1 style='color:{G}'>🗺️ Geographic Distribution</h1>", unsafe_allow_html=True)
    st.info("📍 Bubble maps — size & color represent metric magnitude per state/district")
    ...
    st_i = agg_i.groupby("state")[["insurance_count","insurance_amount"]].sum().reset_index()

    tab1,tab2,tab3 = st.tabs(["💰 Transaction Map","👥 User Map","🛡️ Insurance Map"])
    with tab1:
    ...
    with tab3:
        st.plotly_chart(bubble_map(st_i,"insurance_count","insurance_amount","🛡️ State-wise Insurance Premium"),use_container_width=True)
...
# ── PAGE 6: BUSINESS CASES ───────────────────────────────────────────
    ...
    cases = {
        "Q1 — Top 10 States by Txn Amount":
            "SELECT state, SUM(transaction_count) total_count, SUM(transaction_amount) total_amount FROM agg_transaction GROUP BY state ORDER BY total_amount DESC LIMIT 10",
        "Q2 — Brand-wise Users":
            "SELECT brand, SUM(registered_users) total_users, ROUND(AVG(percentage*100),2) avg_share FROM agg_user GROUP BY brand ORDER BY total_users DESC",
        "Q3 — Insurance Growth by Quarter":
            "SELECT year, quarter, SUM(insurance_count) total_policies, SUM(insurance_amount) total_premium FROM agg_insurance GROUP BY year,quarter ORDER BY year,quarter",
        "Q4 — Top 10 Districts by Users":
            "SELECT state, district, SUM(registered_users) users FROM map_user GROUP BY state,district ORDER BY users DESC LIMIT 10",
        "Q5 — Top 5 Txn Types by Amount":
            "SELECT transaction_type, SUM(transaction_count) count, SUM(transaction_amount) amount FROM agg_transaction GROUP BY transaction_type ORDER BY amount DESC LIMIT 5",
        "Q6 — Yearly Txn Trend":
            "SELECT year, SUM(transaction_count) count, SUM(transaction_amount) amount FROM agg_transaction GROUP BY year ORDER BY year",
        "Q7 — Top 10 Districts by Txn Amount":
            "SELECT state, district, SUM(transaction_count) total_count, SUM(transaction_amount) total_amount FROM map_transaction GROUP BY state,district ORDER BY total_amount DESC LIMIT 10",
        "Q8 — Top 10 States by App Opens":
            "SELECT state, SUM(app_opens) app_opens FROM map_user GROUP BY state ORDER BY app_opens DESC LIMIT 10",
        "Q9 — Top 10 Districts by Insurance":
            "SELECT state, district, SUM(insurance_count) policies, SUM(insurance_amount) premium FROM map_insurance GROUP BY state,district ORDER BY premium DESC LIMIT 10",
    }

    for title, sql in cases.items():
