...
engine = create_engine("mysql+mysqlconnector://root:Pr%40santh001@localhost/phonepe_pulse")
...
agg_txn["avg_txn"] = (agg_txn["transaction_amount"] / agg_txn["transaction_count"].replace(0, np.nan)).round(2)
agg_ins["period"]  = agg_ins["year"].astype(str) + "-Q" + agg_ins["quarter"].astype(str)
agg_ins["avg_premium"] = (agg_ins["insurance_amount"] / agg_ins["insurance_count"].replace(0, np.nan)).round(2)
...
# ── CHART 8: Top 5 Brands Share (Pie) ──
top5_brands = brand_df.head(5).copy()
others_sum  = brand_df.iloc[5:]["registered_users"].sum()
top5_brands = pd.concat([top5_brands, pd.DataFrame([{"brand":"Others","registered_users":others_sum}])], ignore_index=True)
fig, ax = plt.subplots(figsize=(8,6))
ax.pie(top5_brands["registered_users"], labels=top5_brands["brand"],
       autopct="%1.1f%%", startangle=140, colors=sns.color_palette("Set3", len(top5_brands)))
ax.set_title("Top 5 Brands + Others — User Share", fontsize=14, fontweight="bold")
save(fig, "08_brand_pie")
...
# ── CHART 15: Avg Transaction Value by State (Top 10) ──
avg_txn_state = agg_txn.groupby("state").apply(
    lambda x: (x["transaction_amount"].sum() / x["transaction_count"].sum())
).reset_index(name="avg_txn_value").nlargest(10, "avg_txn_value")
fig, ax = plt.subplots(figsize=(12,5))
sns.barplot(data=avg_txn_state, x="state", y="avg_txn_value", palette="YlOrRd", ax=ax)
ax.set_title("Top 10 States by Avg Transaction Value (₹)", fontsize=14, fontweight="bold")
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x,_: f"₹{x:,.0f}"))
plt.xticks(rotation=45, ha="right")
save(fig, "15_avg_txn_value_state")

# ── CHART 16: Heatmap — State × Year Transaction Amount ──
heat_data = agg_txn.groupby(["state","year"])["transaction_amount"].sum().unstack(fill_value=0)
heat_data = heat_data.loc[heat_data.sum(axis=1).nlargest(15).index]
fig, ax = plt.subplots(figsize=(12,7))
sns.heatmap(heat_data/1e12, annot=True, fmt=".1f", cmap="YlOrRd", ax=ax,
            cbar_kws={"label":"₹ Trillion"})
ax.set_title("State × Year Transaction Amount Heatmap (₹ Trillion)", fontsize=13, fontweight="bold")
save(fig, "16_state_year_heatmap")

# ── CHART 17: Quarter-wise Seasonality (Avg across all years) ──
seasonal = agg_txn.groupby("quarter")["transaction_amount"].mean().reset_index()
fig, ax = plt.subplots(figsize=(7,4))
ax.bar(["Q1","Q2","Q3","Q4"], seasonal["transaction_amount"]/1e12,
       color=sns.color_palette("pastel",4), edgecolor="black")
ax.set_title("Average Transaction Amount by Quarter (Seasonality)", fontsize=13, fontweight="bold")
ax.set_ylabel("Avg Amount (₹ T)")
save(fig, "17_quarterly_seasonality")
...
top_state    = state_txn.nlargest(1,"transaction_amount")["state"].values[0]
top_brand    = brand_df.nlargest(1,"registered_users")["brand"].values[0]
